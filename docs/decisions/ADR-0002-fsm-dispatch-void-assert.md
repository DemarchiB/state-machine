...
- `fsm_err_t` como retorno (primeira versão da interface): descartada — o padrão de "construtor valida e retorna erro" do guia é pensado pra parâmetros com faixa de valor real (ex.: corrente em mA), não pra ponteiros que ou existem ou são bug. Manter o retorno de erro obriga a checar (ou ignorar, silenciosamente) um código em todo `fsm_dispatch`.
